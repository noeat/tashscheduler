...
class task<INT, void (C::*)(_Arg...), _Arg...> : public task_base<INT>
{
...
public:
	...
	task_t scheduler_task(INT elapse, int repeat, task_t task);

	template<typename _Fty, typename ..._Arg>
	task_t scheduler(INT elapse, int repeat, _Fty&& fty, _Arg&& ...args);

	template<typename C, typename ..._Arg>
	task_t scheduler(INT elapse, int repeat, C* c, void (C::*)(_Arg...), _Arg&& ...args);

	void cancel_shceduler(task_t task);

	template<typename _Fty, typename ..._Arg>
	task_t create_task(_Fty&& fty, _Arg&& ...args);

	template<typename C, typename ..._Arg>
	task_t create_task(C* c, void (C::*fty)(_Arg&&...), _Arg&&... args);
...
task_scheduler<INT, TVR_BITS, TVN_BITS>::scheduler_task(INT elapse,
...
template<typename _Fty, typename ..._Arg>
typename task_scheduler<INT, TVR_BITS, TVN_BITS>::task_t
task_scheduler<INT, TVR_BITS, TVN_BITS>::scheduler(INT elapse, 
	int repeat, _Fty&& fty, _Arg&& ...args)
{
	...
	assert(task != NULL);
	return scheduler_task(elapse, repeat, task);
}
...
typename task_scheduler<INT, TVR_BITS, TVN_BITS>::task_t
task_scheduler<INT, TVR_BITS, TVN_BITS>::scheduler(INT elapse, 
	int repeat, C* c, void (C::*fty)(_Arg...), _Arg&& ...args)
{
	internal_task<void(C::*)(_Arg...), _Arg...>* task = 
		new internal_task<void(C::*)(_Arg...), _Arg...>(
		c, fty, std::forward<_Arg>(args)...);
	assert(task != NULL);
	return scheduler_task(elapse, repeat, task);
}
...
template<typename _Fty, typename ..._Arg>
typename task_scheduler<INT, TVR_BITS, TVN_BITS>::task_t
	task_scheduler<INT, TVR_BITS, TVN_BITS>::create_task(_Fty&& fty, _Arg&& ...args)
{
	task<INT, _Fty, _Arg...>* task = new ::task<INT, _Fty, _Arg...>(
		std::forward<_Fty>(fty), std::forward<_Arg>(args)...);
	assert(task != NULL);
	return task;
}
...
template<typename INT, int TVR_BITS, int TVN_BITS>
void task_scheduler<INT, TVR_BITS, TVN_BITS>::destory_task(task_t task)
{
	assert(task != nullptr);
	delete task;
}
...
template<typename C, typename ..._Arg>
typename task_scheduler<INT, TVR_BITS, TVN_BITS>::task_t
task_scheduler<INT, TVR_BITS, TVN_BITS>::create_task(C* c, void (C::*fty)(_Arg&&...), _Arg&&... args)
{
	task<INT, void(C::*)(_Arg...), _Arg...> *task =
		new ::task<INT, void(C::*)(_Arg...), _Arg...>(c, fty, std::forward<_Arg>(args)...);
	assert(task != nullptr);
	return task;
}
